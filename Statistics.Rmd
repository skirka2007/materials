...
Now we want to decide if this data is good enough for the t-test. According to the wiki, the following assumptions should be met:
...
2.  The two populations being compared should have the same variance. If the sample sizes in the two groups being compared are equal, Student's original t-test is highly robust to the presence of unequal variances.

Our sample sizes are equal and enough large to use the T-test. But to prove the assumptions we can use the bootstrap method and Q-Q-plot.

### Bootstrap
Bootstrapping is a method that estimates the sampling distribution by taking multiple samples with replacement from a single random sample. These repeated samples are called resamples.
